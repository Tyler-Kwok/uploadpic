function up_arr(input, options) {
	...
	this.add = function() {
		...
		if ($this.settings.multi && $this.input.prop("files").length > 1) {
			$this.multiAdd($this.input.prop("files"));
		} else {
			$this.singleAdd($this.input.prop("files")[0]);
		}
		}
	};
	...
	if (this.settings.progress)
		this.xhr.upload.addEventListener('progress', this.settings.progress,
				false);
